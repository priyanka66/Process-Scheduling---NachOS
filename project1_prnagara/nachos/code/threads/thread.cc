...
Thread::Fork(VoidFunctionPtr func, void *arg)
{
    ...
    kernel->currentProcess->setThreadList(this);
    this->setStatus(READY);

  //  scheduler->ProcessReadyToRun(kernel->currentProcess);	// ReadyToRun assumes that interrupts 
					// are disabled!
...
Thread::Finish ()
{
    ...
    Sleep(kernel->currentProcess,TRUE);				// invokes SWITCH
    // not reached
}

void
Thread::Finish (Process *p)
{   
    (void) kernel->interrupt->SetLevel(IntOff);         
    ASSERT(this == kernel->currentProcess->currentThread);
...
void
Thread::Sleep (Process *p, bool finishing)
{
    Thread *nextThread;
    Process *nextProcess;
    ASSERT(this == kernel->currentProcess->currentThread);
    ...
    status = BLOCKED;
    cout << "\n ^^^ READY LIST FOR PROCESS ^^ " << p->getName() << "  " ;
    if ((nextThread = p->scheduler->FindNextThreadToRun()) != NULL) {
        p->scheduler->Run(nextThread, true);
...
static void ThreadFinish()    { kernel->currentProcess->currentThread->Finish(kernel->currentProcess); }
static void ThreadBegin() { kernel->currentProcess->currentThread->Begin(); }
void ThreadPrint(Thread *t) { t->Print(); }
void ThreadIDPrint(Thread *t) { t->PrintID(); cout << "  " ;  }
#ifdef PARISC

//----------------------------------------------------------------------
// PLabelToAddr
//	On HPUX, function pointers don't always directly point to code,
///Begin	so we need to do the conversion.
//----------------------------------------------------------------------
